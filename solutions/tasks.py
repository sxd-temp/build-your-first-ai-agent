"""SOLUTION — Task definitions for the Startup Idea Validator crew.

Don't peek until you've tried! Copy build_bear_task here into src/tasks.py if
you get stuck or fall behind.
"""
...
def build_bear_task(
    ...
    return Task(
        description=(
            ...
            f"{_idea_block(idea, customer, advantage, budget)}\n\n"
            f"Use the search and scrape tools to research:\n"
            f"1. Existing incumbents and well-funded competitors.\n"
            f"2. Failed predecessors — startups that tried this and died (find post-mortems "
            f"if possible).\n"
            f"3. Structural problems: distribution, unit economics, regulatory, "
            f"customer acquisition cost.\n"
            f"4. Why customers might say they want this but never actually pay.\n"
            f"5. Why the founder's stated advantage might not matter at scale.\n\n"
            f"Be brutally honest, never cruel. Cite every claim with a URL. If a "
            f"weakness is hypothetical, label it as a hypothesis."
        ),
        expected_output=(
            "A structured Markdown brief titled `# 🔪 Bear Case` with these sections:\n"
            "- **Top Reasons It'll Fail** (3-5 bullets, each ending with a source link)\n"
            "- **Incumbents & Competitors** (who already owns this space)\n"
            "- **Failed Predecessors** (with links to post-mortems where possible)\n"
            "- **Structural Risks** (distribution, unit economics, CAC, regulation)\n"
            "- **Customer Apathy Risk** (why they won't pay even if they say they will)\n"
            "- **Sources** (numbered list of all URLs)"
        ),
        agent=agent,
    )
